...
### 🐛 修复
- **修复自动保存产生大量时间戳垃圾会话的问题**
  - `_cmd_new` 未重置 `_auto_titled` 标志位，导致新对话自动标题被跳过，每轮保存都生成新时间戳会话
  - `_do_save` 中 `generate_session_title` 失败时无兜底，回退到时间戳命名
- **`_do_save` 加失败冷却+降级兜底**
  - API 标题生成失败后 1 分钟内不重复请求
  - 多次失败后降级到首条消息前 15 字作为会话名
  - 实在无名字时不保存，彻底杜绝时间戳
- **`_cmd_new` 保存旧会话时加 `if session_name:` 保护**
  - 避免 API 生成标题失败后以空名字调用 `save_session` 产生垃圾
